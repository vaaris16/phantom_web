...
export default function SponsorsInfo() {
  return (
    <section className="pt-24 flex flex-col justify-center items-center">
      ...
      <p className="pt-5 ml-3 md:ml-0 md:text-center font-Montserrat font-medium text-(--secondary-color)">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 mt-20 mb-20 gap-6">
        {SponsorsList.map(({ name, description, website, image, level }) => (
          <div className="relative group">
            <div
              aria-hidden="true"
              className="absolute w-75 h-110 md:w-150 md:h-60 rounded-4xl blur-xs md:blur-xl opacity-100 md:opacity-0 bg-phantom transition-all duration-500 group-hover:opacity-100 group-hover:-translate-y-3"
            ></div>
            <div className="relative w-75 h-110 md:w-150 md:h-60 bg-(--cards-color) py-3 md:py-6 px-6 flex flex-col md:flex-row transition-all duration-500 group-hover:opacity-100 group-hover:-translate-y-3 items-center rounded-4xl">
              <img
